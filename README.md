...
No, it does not return an error.
- **What log message appears in the terminal?**  
The terminal log shows Query params received -> status: 'INVALID'... followed by Returned 0 matching records from MongoDB

**Test 2: Invalid Priority Value**

- **Does your API support this priority?**  
No, the API does not logically support a priority value of "URGENT"
- **What response is returned?**  
Returns an empty array [] with a 200 OK status
...
- **Does the API crash?**  
No, the API does not crash
- **Does it return an empty page?**  
Yes, it returns a standard 200 OK response where the payload's content array is completely empty ([]).
- **What does the page metadata show?**  
The metadata object shows "empty": true, "number": 99, and "numberOfElements": 0.

**Test 4: Very Large Page Size**

- **Does the API allow this?**  
Yes, the API allows this without throwing any errors or warnings.

- **Should real APIs allow very large page sizes?**  
No, production APIs should never allow unrestricted page sizes.
...
Yes, it returns data successfully with a 200 OK status.

- **Does the sorting seem meaningful?**  
No. MongoDB cannot sort by a property that does not exist in the documents.

- **Should the backend validate allowed sort fields?**  
Yes. The backend should explicitly validate that incoming sortBy values match existing fields
...
It only applies the first filter parameter matched in the execution chain

- **Is this behaviour clear to the API user?**  
NO, it is highly confusing.

### Reflection Questions

**1. What happened when you used an invalid status?**  
The API processed the request successfully with a 200 OK status but returned an empty JSON list ([])

**2. What happened when you used an invalid priority?**  
It returned a 200 OK code with an empty array.

**3. What happened when you requested page 99?**  
The API generated a valid pagination metadata block tracking the position as page 99 but left the interior content array empty without crashing.

**4. What happened when you used an unknown sort field?**  
The query ran smoothly, but sorting failed silently.

**5. Why should an API limit page size?**  
To prevent memory starvation

**6. Why should an API validate sort fields?**  
To prevent inefficient collection scans

**7. Does your current API support combined filters?**  
No

**8. What log messages helped you understand what happened?**  
The SLF4J tracer logs generated by TicketService showed exactly which branch was reached

**9. Which behaviour would you improve in a future version?**  
I would rewrite the query matching logic using a dynamic Spring Data MongoDB Criteria array builder.

## Day 10 Exercise 02 - Ticket Report by Status
