...
const server = http.createServer(async (request, response) => {
    ...
    try {
        ...
        const singleEventMatch = url.pathname.match(/^\/api\/events\/([^/]+)$/);
        ...
        if (method === "POST" && url.pathname === "/api/bookings") {
            ...
            const targetEvent = events.find(e => e.id === payload.eventId.trim());
            if (!targetEvent) {
                sendJson(response, 404, { message: `Event ${payload.eventId} was not found` });
                return;
            }

            if (targetEvent.availableSeats < payload.seats) {
                sendJson(response, 400, { message: "Not enough seats available" });
                return;
            }

            // Deduct allocated capacity seats safely
        ...
        if (method === "DELETE" && bookingMatch) {
            ...
            if (foundBooking.status === "CANCELLED") {
                sendJson(response, 400, { message: "Booking is already cancelled" });
                return;
            }

            const relatedEvent = events.find(e => e.id === foundBooking.eventId);
            if (relatedEvent) {
                relatedEvent.availableSeats += foundBooking.seats;
            }

            foundBooking.status = "CANCELLED";
            sendJson(response, 200, foundBooking);
            return;
        }
...
server.listen(PORT, () => {
    ...
    console.log(`🔗 Health check active: http://localhost:${PORT}/api/health`);
    console.log(`🔗 Events endpoints active: http://localhost:${PORT}/api/events\n`);
});